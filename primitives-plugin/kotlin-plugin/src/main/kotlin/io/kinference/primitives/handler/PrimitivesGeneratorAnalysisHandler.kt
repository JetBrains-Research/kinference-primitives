...
class PrimitivesGeneratorAnalysisHandler(private val outputDir: File) : AnalysisHandlerExtension {
    ...
    override fun doAnalysis(
        ...
        componentProvider.get<LazyTopDownAnalyzer>().analyzeDeclarations(TopDownAnalysisMode.TopLevelDeclarations, primitiveFiles)
...
//        val context = ResolveUtil.analyze(ktFiles, manager).bindingContext
        ...
        val primitiveFiles = files.filter { it.isAnnotatedWith<GenerateWithPrimitives>(context) }
        val primitiveClasses = primitiveFiles.flatMap { it.collectClasses(context) }
    ...
    companion object {
        ...
        private val PRIMITIVE_CLASS = FqName(PrimitiveClass::class.qualifiedName!!)
    }
}
